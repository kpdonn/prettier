            ...
            case MODE_BREAK:
              ...
              if (doc.literal) {
                ...
                if (out.length > 0) {
                  ...
                  if (
                    out.length &&
                    (options.parser !== "markdown" || // preserve markdown's `break` node (two trailing spaces)
...
function amountPrintedBeforeNumBreaks(
  next,
  restCommands,
  width,
  pos,
  ...
  while (breaks > 0) {
    if (cmds.length === 0) {
      if (restIdx === 0) {
        return amountPrinted + breaks * width;
      }
      cmds.push(restCommands[restIdx - 1]);
    ...
    if (typeof doc === "string") {
      amountPrinted += util.getStringWidth(doc.trim());
      pos += util.getStringWidth(doc);
    } else {
      switch (doc.type) {
        case "concat":
