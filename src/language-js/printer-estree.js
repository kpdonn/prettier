...
const chooseGroup = docBuilders.chooseGroup;
...
function printAssignmentRight(rightNode, printedRight, canBreak, options) {
  ...
  const noBreakGroup = concat([" ", printedRight]);
  const breakGroup = concat([
    " ",
    "(",
    indent(concat([softline, printedRight])),
    softline,
    ")"
  ]);
  return chooseGroup(noBreakGroup, breakGroup, 2, 1.05, 2);
}
